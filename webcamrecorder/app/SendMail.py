...
class SendMail(ISendMail.ISendMail):
    def __init__(self) -> None:
        ...
        cur.execute("create table if not exists mail_histories(date DATETIME primary key)")

    def send(self, frame: Frame, subect: str = "subject", body: str = "body"):
        if self.sender.can_send == False:
                return

        cur = self.conn.cursor()

        results = cur.execute("select date from mail_histories order by date desc limit 1")
    ...
    def insert_history(self) -> None:
        cur = self.conn.cursor()
        cur.execute("delete from mail_histories where date not in (select date from mail_histories order by date desc limit 100)")
